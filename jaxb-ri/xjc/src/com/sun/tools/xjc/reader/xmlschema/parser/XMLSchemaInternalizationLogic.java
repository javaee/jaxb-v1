...
public class XMLSchemaInternalizationLogic implements InternalizationLogic {
    ...
    private class ReferenceFinder extends AbstractReferenceFinderImpl {
        ...
        protected String findExternalResource( String nsURI, String localName, Attributes atts) {
            if( Const.XMLSchemaNSURI.equals(nsURI)
    ...
    public boolean checkIfValidTargetNode(DOMForest parent, Element bindings, Element target)  throws SAXException {
        return Const.XMLSchemaNSURI.equals(target.getNamespaceURI());
    }

    public Element refineTarget(Element target) {
        // look for existing xs:annotation
        Element annotation = DOMUtils.getFirstChildElement(target, Const.XMLSchemaNSURI, "annotation");
        if(annotation==null)
            // none exists. need to make one
            annotation = insertXMLSchemaElement( target, "annotation" );
        
        // then look for appinfo
        Element appinfo = DOMUtils.getFirstChildElement(annotation, Const.XMLSchemaNSURI, "appinfo" );
        if(appinfo==null)
            // none exists. need to make one
    ...
    private Element insertXMLSchemaElement( Element parent, String localName ) {
        ...
        Element child = parent.getOwnerDocument().createElementNS( Const.XMLSchemaNSURI, qname );
